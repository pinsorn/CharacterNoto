...
export function buildPublicView(state, share, hides = {}) {
  ...
  if (share.map) {
    ...
    v.mapData = { backgroundId: md.backgroundId ?? null, regions: keep(md.regions), tokens: keep(md.tokens), objects: md.objects || [] };
  }
...
export async function applySnapshot(snap) {
  ...
  mapData.set(s.map ? v.mapData || { backgroundId: null, regions: [], tokens: [], objects: [] } : { backgroundId: null, regions: [], tokens: [], objects: [] });
